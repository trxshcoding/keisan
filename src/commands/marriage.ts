...
export default declareCommand({
  ...
  async button(interaction, config) {
    const proposal = proposals.find(
      (p) => p.interaction.id === interaction.message.interactionMetadata!.id,
    );
    if (!proposal) return;
    ...
    switch (interaction.customId) {
      case "yes": {
        ...
        await config.prisma.user.upsert({
          where: { id: proposal.from },
          update: {},
          create: { id: proposal.from },
        });
        await config.prisma.user.upsert({
          where: { id: proposal.to },
          update: {},
          create: { id: proposal.to },
        });
  ...
  slashCommand: new SlashCommandBuilder()
    .setName("marriage")
    .setDescription("so romantic")
    .setIntegrationTypes([ApplicationIntegrationType.UserInstall])
    .addSubcommand(
      new SlashCommandSubcommandBuilder().setName("status").setDescription("get marriage status"),
    )
    .addSubcommand(
      new SlashCommandSubcommandBuilder()
        ...
        .addUserOption(
          new SlashCommandUserOption().setName("user").setDescription("new wife").setRequired(true),
        ),
    )
    .addSubcommand(
      new SlashCommandSubcommandBuilder()
