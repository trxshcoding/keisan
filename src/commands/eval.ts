...
function transformStatement<T extends Statement | ModuleDeclaration>(ast: T): T | Statement {
  switch (ast.type) {
    ...
    case "IfStatement":
      ...
      if (ast.alternate) ast.alternate = transformStatement(ast.alternate);
      break;
  }
...
export default declareCommand({
  ...
  async run(interaction: ContextMenuCommandInteraction, target: Message): Promise<void> {
    ...
    const bindings: { name: string; value: any }[] = [
      ...
      {
        name: "getUser",
        value: (snowflake: UserResolvable) => interaction.client.users.fetch(snowflake),
      },
    ];
    // eslint-disable-next-line no-implied-eval
    const func = new Function(
      ...bindings.map((it) => it.name),
      `"use strict";\nreturn (async () => {\n${mappedCode}\n})();`,
    );
