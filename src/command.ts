...
export function declareCommand<ExtraConfig, T extends User | Message>(
  command: OmitPrime<ContextCommand<T, ExtraConfig>, keyof Brand>,
): ContextCommand<T, ExtraConfig>;
export function declareCommand<ExtraConfig>(
  command: OmitPrime<Command<ExtraConfig>, keyof Brand>,
): Command<ExtraConfig>;
export function declareCommand<ExtraConfig>(
  command: OmitPrime<AnyCommand<ExtraConfig>, keyof Brand>,
): AnyCommand<ExtraConfig> {
  const t = Object.assign(command, { [commandSymbol]: true } as const) satisfies Omit<
    AnyCommand<ExtraConfig>,
    keyof Brand
  > &
    Brand;
...
export function undeclareCommand(obj: unknown): AnyCommand<unknown> {
  if (typeof obj !== "object" || !obj || !Object.hasOwn(obj, commandSymbol))
    throw new Error(`${String(obj)} is not a command`);
...
export type ContextCommand<T extends User | Message, ExtraConfig> = ICommand<ExtraConfig> & {
  ...
  run(
    ...
    target: T extends User ? User : T extends Message ? Message : never,
    config: Config,
  ): Promise<void>;
  commandName: string;
...
export type AnyCommand<ExtraConfig> =
  | Command<ExtraConfig>
  | ContextCommand<User | Message, ExtraConfig>;

export type PackagedCommand<T extends AnyCommand<ExtraConfig>, ExtraConfig> = T & {
  storedConfig: ExtraConfig;
};

export function tryPackageCommand<T extends AnyCommand<ExtraConfig>, ExtraConfig>(
  command: T,
): PackagedCommand<T, ExtraConfig> {
